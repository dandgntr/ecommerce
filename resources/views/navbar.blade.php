<header class="text-gray-600 body-font sticky top-0 bg-white z-50 shadow-md">
  <div class="container mx-auto flex flex-wrap py-3 px-5 md:flex-row items-center">
    <a class="flex title-font font-medium items-center text-gray-900">
      <img src="{{ asset('image/logo1.png') }}" alt="Logo" class="w-10 h-10 rounded-full">
    ...
    <nav class="md:mr-auto md:ml-6 md:py-1 md:pl-4 md:border-l md:border-gray-400 flex flex-wrap items-center text-base justify-center space-x-5">
      ...
      <a href="{{ route('product') }}" class="hover:text-gray-900">Product</a>
    ...
    <div class="flex space-x-4 items-center ml-auto">
      ...
      @guest
        <a href="{{ route('login') }}" class="inline-flex items-center bg-indigo-500 text-white border-0 py-1 px-3 focus:outline-none hover:bg-indigo-600 rounded text-base">Login</a>
        <a href="{{ route('register') }}" class="inline-flex items-center bg-gray-100 text-gray-900 border-0 py-1 px-3 focus:outline-none hover:bg-gray-200 rounded text-base">Register</a>
      @endguest
      ...
      @auth
        <div class="relative ml-3" x-data="{ open: false }">
          ...
          <div x-show="open" @click.away="open = false" class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button">
            ...
            <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem">Logout</a>
            <!-- Form Logout -->
